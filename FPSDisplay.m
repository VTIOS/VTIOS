...
- (void)initDisplayLabel {
    ...
    _font = [UIFont fontWithName:@"Menlo" size:40]; // Tăng kích thước font chính
    if (_font) {
        _subFont = [UIFont fontWithName:@"Menlo" size:14]; // Tăng kích thước font phụ
    } else {
        _font = [UIFont fontWithName:@"Courier" size:40]; // Tăng kích thước font chính
        _subFont = [UIFont fontWithName:@"Courier" size:14]; // Tăng kích thước font phụ
    }
...
- (void)updateDisplayLabelText:(float)fps {
    ...
    self.displayLabel.font = [UIFont italicSystemFontOfSize:14]; // Tăng font phụ lên

    NSString *WMText = [NSString stringWithFormat:@" %d FPS | %@ | Pin: %0.0f  zKuTom Roblox", (int)round(fps), mustr, batLeft]; 
